...
public class TriggerInstances extends AbstractInstances {
	...
	protected List<String> getLabelsString() {
		...
		List<String> tokenTypes = new Configuration("").getEventTypeNames();
	...
	protected Instance tokenToInstance(JCas jcas, Token token,
		...
		if (null != triggerTokens) {
			...
			instance.setLabelString(triggerTokens.containsKey(token.getId()) ? triggerTokens
					.get(token.getId()) : "Non_trigger");
		} else {
			instance.setLabelString("Non_trigger");
		}
	...
	public static void main(String[] args) {
		...
		List<Instance> instances = ti.getInstances(new File(args[0]), "Trigger");
